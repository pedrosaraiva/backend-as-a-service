    ...
    $.getJSON('data/companies.json', function(data) {  	
        $.each(data['company'], function(key, val) {
        	...
        	if(thisid == id)
	            ...
	            github = val['github'];
	        	if(github!=''){ document.getElementById("github-icon").href=github; alert("1)"+github);  } else { document.getElementById("github-icon-img").style.display=='hidden';alert("1)"+github); }	        		        	                
            	}
          	});                                    
        });
    }  